...
public class FileIO {
	...
	public Properties getPropertiess(String fileName)
	{
	...
	public Automobile buildAutoFromProperties(String fileName)
	{
		...
		try {
			...
			if(!name.equals(null))
			{
				...
				int optionSetCount = Integer.parseInt(props.getProperty("OpionSetCount"));
								
				for(int i = 0; i < optionSetCount; i++)
				{
					String optionSetName = props.getProperty("OptionSet" + i + 1);
					int optionCount = Integer.parseInt(props.getProperty("OptionSet" + i + 1 + "Count"));
			...
			}//end of if name is equal to null
